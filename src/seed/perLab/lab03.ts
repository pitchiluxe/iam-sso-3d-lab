...
export function applyLab03Seed(
  ...
  tickets.create({
    ...
    payload: {
      userId: jane,
      requestedRoleIds: [
        base.roleIds['role-finance-payroll-writer'] ?? base.roleIds['grp-finance-payroll']!,
      ],
      justification: 'New financial analyst joining payroll operations',
    },
  });

  // Fault: Bob has standing domain admin role
  const adminRole = dir.createRole(
    'role-domain-admins',
